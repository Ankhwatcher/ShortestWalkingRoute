...
public class LocationService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent != null) {
			...
			SharedPreferences settings = getSharedPreferences(
					ie.appz.shortestwalkingroute.CaptureRouteActivity.PREFS_NAME,
					0);
		...
		mLocationManager.requestLocationUpdates(
				LocationManager.NETWORK_PROVIDER, 10000, 5, networkListener);
	...
	protected void addRow(Location location) {
		...
		if (location.getAccuracy() < 100
				&& location.getAccuracy() < 2 * oldAccuracy) {
			Log.d(LocationService.class.getName(), "Adding fix to "
					+ FixOpenHelper.FIX_TABLE_NAME + " in route number "
			...
			Uri baseUri = Uri.withAppendedPath(FixProvider.CONTENT_URI,
					FixProvider.ROUTE + "/" + mRouteNo);
